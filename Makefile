...
ready-deployment: kind-image deploy-kcp apibinding ## Deploy the controller-manager and wait for it to be ready.
	$(KCP_KUBECTL) --namespace "internal-services-controller-system" rollout status deployment/internal-services-controller-controller-manager
...
apibinding:
	...
	$(KCP_KUBECTL) wait --for=condition=Ready apibinding/internal-services-controller-internal-services-controller.appstudio.redhat.com

.PHONY: kind-image
kind-image: docker-build ## Load the controller-manager image into the kind cluster.
	kind load docker-image $(REGISTRY)/$(IMG) --name e2e-internal-services-controller

$(ARTIFACT_DIR)/kind.kubeconfig: $(ARTIFACT_DIR) ## Run a kind cluster and generate a $KUBECONFIG for it.
	@if ! kind get clusters --quiet | grep --quiet e2e-internal-services-controller; then kind create cluster --name e2e-internal-services-controller; fi
	kind get kubeconfig --name e2e-internal-services-controller > $(ARTIFACT_DIR)/kind.kubeconfig
...
kcp-synctarget: kcp-workspace $(ARTIFACT_DIR)/syncer.yaml yq ## Add the kind cluster to kcp as a target for workloads.
	...
	$(KCP_KUBECTL) wait --for=condition=Ready synctarget/kind-e2e-internal-services-controller
...
$(ARTIFACT_DIR)/syncer.yaml: ## Create the SyncTarget and generate the manifests necessary to register the kind cluster with kcp.
	$(KCP_KUBECTL) kcp workload sync kind-e2e-internal-services-controller --resources services --syncer-image ghcr.io/kcp-dev/kcp/syncer:v$(KCP_VERSION) --output-file $(ARTIFACT_DIR)/syncer.yaml

.PHONY: kcp-workspace
kcp-workspace: $(KUBECTL_KCP) kcp-server ## Create a workspace in kcp for the controller-manager.
	sleep 10
	$(KCP_KUBECTL) kcp workspace use '~'
	@if ! $(KCP_KUBECTL) kcp workspace use internal-services-controller; then $(KCP_KUBECTL) kcp workspace create internal-services-controller --type universal --enter; fi
...
test-e2e-cleanup: ## Clean up processes and directories from an end-to-end test run.
	kind delete cluster --name e2e-internal-services-controller || true
...
NAME_PREFIX ?= internal-services-controller
APIEXPORT_NAME ?= appstudio.redhat.com
