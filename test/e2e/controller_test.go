...
import (
	...
	appstudioredhatcomv1alpha1 "github.com/hacbs-release/internal-services-controller/api/v1alpha1" //+kubebuilder:scaffold:imports
)
...
func loadClient(t *testing.T, clusterName logicalcluster.Name) client.Client {
	...
	if err := appstudioredhatcomv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to add %s to scheme: %v", appstudioredhatcomv1alpha1.GroupVersion, err)
	}
...
func createAPIBinding(t *testing.T, workspaceCluster logicalcluster.Name) client.Client {
	...
	apiName := "internal-services-controller-internal-services-controller.appstudio.redhat.com"
...
func TestController(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		t.Run(fmt.Sprintf("attempt-%d", i), func(t *testing.T) {
			...
			// }
			// if err := c.Create(context.TODO(), &appstudioredhatcomv1alpha1.Request{
			//     ObjectMeta: metav1.ObjectMeta{Namespace: namespaceName, Name: fmt.Sprintf("resource-%d", i)},
			//     Spec: appstudioredhatcomv1alpha1.RequestSpec{},
			// }); err != nil {
